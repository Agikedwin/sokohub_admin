...
class MeasurementController extends TBaseController<MeasurementModel>{
  ...
  final RxList<MeasurementModel> selectedMeasurement =
  ...
  void sortByName(int sortColumnIndex, bool ascending){
    sortByProperty(sortColumnIndex, ascending, (MeasurementModel material) => material.name.toLowerCase());
  }
    ...
    Future<List<MeasurementModel>> loadSelectedMeasuremnts(String garmentId) async{
    measurementLoading.value = true;

    // Product categories
    final garmentMeasurement = await measurementRepository.getAllGarmentMeasuremnt(garmentId);

    if(allItems.isEmpty) await fetchItems();

    final measurementIds = garmentMeasurement.map((e) => e.measurementId).toList();
    final measurements  = allItems.where((element)=> measurementIds.contains(element.id)).toList();
   ...
   Future<void> addGarmentMeasurement() async{
    try {
      ...
      List<String> existingMeasurementIds = alreadySelectedMeasurement.map((measurement) => measurement.id).toList();
        ...
        if(!existingMeasurementIds.contains(measurement.id)){
         ...
         final garmentMeasurement = GarmentMeasurementModel(
          ...
          garmentId: GarmentController.instance.selectedGarment.value.id,
           ...
           alreadySelectedMeasurement.add(selectedMeasurement.firstWhere((item) => item.id == measurement.id));
      ...
      for (var existingMeasurementId in existingMeasurementIds) {
        // Check if the Measurement is not already associated with the Garment
        if(!selectedMeasurement.any((access) => access.id == existingMeasurementId)){
          // Remove the association
          await  measurementRepository.deleteAAssociatedccessory(existingMeasurementId, GarmentController.instance.selectedGarment.value.id);

          alreadySelectedMeasurement.removeWhere((item) => item.id == existingMeasurementId);
          alreadySelectedMeasurement.refresh();
        }
